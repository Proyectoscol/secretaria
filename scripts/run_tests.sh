...
#   ./scripts/run_tests.sh security  # security tests only (stage 2)
#   ./scripts/run_tests.sh unit      # unit tests only (stage 3)
#
# Exit codes:
#   0 — all tests passed
...
echo -e "${BLUE}KOS Test Suite — mode: ${MODE}${NC}"
echo "Root: $REPO_ROOT"
...
print_stage 2 "Security Tests (ClamAV, whitelist, no-bounce, limits)"
...
run_pytest "mail parser" \
  tests/mail/test_parser.py \
  -m "not integration"

run_pytest "mail whitelist" \
  tests/mail/test_whitelist.py \
  -m "not integration"

run_pytest "mail thread builder" \
  tests/mail/test_thread_builder.py \
  -m "not integration"

run_pytest "LLM client" \
...
# ── Stage 4: Existing KOS tests ───────────────────────────────────────────────

print_stage 4 "KOS Core Tests (multitenancy, ingestion stages, librarian)"
run_pytest "kos core" \
  tests/kos/ \
  -m "not integration"
