...
# The frontend runs inside Docker on the kos_default bridge network.
# From inside the container 'localhost' resolves to the container itself, not
# the host. Replace localhost/127.0.0.1 with the Docker service name 'postgres'
# so PrismaAdapter can reach PostgreSQL via Docker's internal DNS.
FRONTEND_DB_URL="${DB_URL//@localhost:/@postgres:}"
FRONTEND_DB_URL="${FRONTEND_DB_URL//@127.0.0.1:/@postgres:}"
...
# AUTH_TRUST_HOST is required in Auth.js v5 when the app runs behind a reverse
# proxy (nginx). Without it every request fails with "UntrustedHost" error.
...
# ── Base de datos ──────────────────────────────────────────────────────────────
# Uses Docker service name 'postgres' (not localhost) — required for bridge networking.
...
# that test fails and certbot aborts. Write a plain HTTP config first; once the
# cert is obtained we swap in the full HTTPS config.
info "Fase 1: configurando Nginx HTTP (para el reto ACME)…"

NGINX_CONF="/etc/nginx/sites-available/kos"
sudo mkdir -p /var/www/certbot
sudo tee "$NGINX_CONF" > /dev/null <<NGINXHTTP
# Generated by scripts/onboarding.sh (HTTP-only — updated after SSL cert is issued)
server {
    listen 80;
    server_name ${DOMAIN} www.${DOMAIN};

    # ACME challenge root — certbot webroot writes here
    location /.well-known/acme-challenge/ {
        root /var/www/certbot;
    }

    # Everything else → temporary plain-HTTP proxy (replaced by 301 after cert)
    location / {
...
# Remove default site if still enabled (conflicts with our server_name)
sudo rm -f /etc/nginx/sites-enabled/default
sudo nginx -t && sudo systemctl reload nginx
success "Nginx HTTP configurado para ${DOMAIN}"
...
# ── Phase 3: upgrade to full HTTPS config (only if cert exists) ───────────────
if [[ -f "/etc/letsencrypt/live/${DOMAIN}/fullchain.pem" ]]; then
  info "Fase 3: activando config HTTPS completa…"
  sudo tee "$NGINX_CONF" > /dev/null <<NGINXHTTPS
...
NGINXHTTPS
  sudo nginx -t && sudo systemctl reload nginx
  success "Nginx HTTPS activado para ${DOMAIN}"
fi
